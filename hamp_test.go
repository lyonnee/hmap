...
func (c mapCall) apply(m *Map[any, any]) (any, bool) {
	switch c.op {
	case opLoad:
		return m.Load(c.k)
	case opStore:
...
func applyCalls(m *Map[any, any], calls []mapCall) (results []mapResult, final map[any]any) {
	for _, c := range calls {
		v, ok := c.apply(m)
...
func applyMap(calls []mapCall) ([]mapResult, map[any]any) {
	return applyCalls(New[any, any](), calls)
}
